...
function renderReadme(data, ruleTitles) {
  ...
  L.push('');
  L.push(`**${mapped.length} code-level exploits** — ${formatMoney(totalMapped)} lost — mapped to **${coveredRules.length} of the 31 rules**. Plus **${outOfScope.length} notable incidents no code rule prevents** (stolen keys, off-chain wallets), listed for honesty about scope.`);
  L.push('');
