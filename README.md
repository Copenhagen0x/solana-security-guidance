...
sha256sum -c CHECKSUMS.txt          # all three must print "OK" — aborts if any byte differs
mkdir -p "$OLDPWD/.claude" && cp claude-security-guidance.md security-patterns.yaml "$OLDPWD/.claude/"
```

Pinning to a tag freezes you to a known release (a tampered `main` can't reach you); `sha256sum -c` confirms nothing was altered in transit. Release tags are **signed** — verify origin with `git verify-tag v1.9.1` (signing key in [`SECURITY.md`](SECURITY.md)). *(Checksums committed in the repo can't defend against a full account compromise that rewrites both the files and the checksums — the signed tag is the cryptographic origin check for that.)*
