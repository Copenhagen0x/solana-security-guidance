...
const EXPECTED_IDS = Array.from({ length: 34 }, (_, i) => 'SOL-0' + String(i + 1).padStart(2, '0'));
...
test('every AI-tool rules file carries all 34 SOL-0XX rules + the behavioral intro', () => {
...
test('README is honest about coverage: 34 documented vs 22 machine-checkable', () => {
  const r = read('README.md');
  assert.match(r, /34 documented SOL-0XX rules/);
  assert.match(r, /22 rules that have/);
