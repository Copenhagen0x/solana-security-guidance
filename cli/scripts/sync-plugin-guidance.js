...
function render() {
  ...
  const lines = [];
  lines.push('<!-- GENERATED by cli/scripts/sync-plugin-guidance.js from claude-security-guidance.md — do not edit by hand. Regenerate: cd cli && npm run sync:plugin-guidance -->');
  lines.push('# Solana Security Standard — by Jelleo');
  lines.push('');
  lines.push('> Compact `SOL-0XX` rule cues for inline AI guidance. **Full per-rule detail** (impact, numbered exclusions, examples): the `list_solana_security_rules` MCP tool, or the complete `claude-security-guidance.md` at github.com/Copenhagen0x/solana-security-standard . Each rule has a stable ID — cite like a CWE.');
  lines.push('');
  lines.push(preamble(md));
  lines.push('');
  lines.push('## Rules — flag the pattern, apply the fix, cite the SOL-0XX id');
  lines.push('');
  for (const r of rules) {
    lines.push(`- **${r.id}** · ${r.title} — ${extractFix(r.body)}`);
  }
  lines.push('');
  return lines.join('\n');
}
